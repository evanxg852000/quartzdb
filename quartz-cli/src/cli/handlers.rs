...
pub async fn handle_run(config: QuartzConfig) -> anyhow::Result<()> {
    ...
    let app = axum::Router::new()
        .route("/", axum::routing::get(|| async { axum::Json(AppInfo::new())}))
...
pub async fn handle_index_list(config: QuartzConfig) -> anyhow::Result<()> {
    ...
    let response = client.get(format!("{}/api/v1/metastore/indexes", config.endpoint))
        .send()
        .await?;
...
pub async fn handle_index_create(config: QuartzConfig, file: PathBuf) -> anyhow::Result<()> {
    ...
    let client = reqwest::Client::new();
    let response = client.post(format!("{}/api/v1/metastore/indexes", config.endpoint))
        .json(&index_meta)
        .send()
        .await?;
    match response.status().is_success() {
...
pub async fn handle_index_delete(config: QuartzConfig, index_name: &str) -> anyhow::Result<()> {
    let client = reqwest::Client::new();
    let response = client
        .delete(format!("{}/api/v1/metastore/indexes/{}", config.endpoint, index_name))
        .send()
        .await?;
    match response.status().is_success() {
        true => {
