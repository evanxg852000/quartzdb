...
pub async fn run_cli() -> anyhow::Result<()> {
    ...
    match command {
        ...
        Some(Commands::Ingest { file }) => handlers::handle_ingest(file).await?,
        Some(Commands::Query { query }) => handlers::handle_query(&query).await?,
...
fn load_config(
    overriden_config: Option<PathBuf>,
    overriden_endpoint: Option<String>,
) -> anyhow::Result<QuartzConfig> {
    ...
    let override_config_file_path =
        overriden_config.unwrap_or_else(|| current_dir.join(CONFIG_FILE_BASE_NAME));
