...
impl SplitWriter {
    pub async fn try_new(table_name: String, storage: Arc<dyn Storage>) -> Result<Self> {
        let split_id = uuid::Uuid::now_v7().to_string();
        let split_dir = storage.root().join(&table_name).join(&split_id);
    ...
    pub async fn finalize(self) -> Result<SplitMeta> {
        // Move index & data into storage /index/<table_name>/{split.idx, split.bin}
    ...
    async fn write_data(&self, table_config: &TableConfig, batch: StorerBatch) -> Result<()> {
        ...
        let num_dynamic_columns = table_config.fields.len();
        let dynamic_colums_types = table_config
            .fields
            .iter()
