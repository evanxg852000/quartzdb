...
pub async fn start_storer_service(storer_config: &StorerConfig, storage_config: &StorageConfig, metastore_client: MetastoreClient) -> Result<StorerServiceStartResult> {
    let storage = storage_config.derive(STORER_DIR, storer_config.cache.clone()).build().await?; 
