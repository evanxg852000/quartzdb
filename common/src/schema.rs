...
impl Schema {
    pub fn get_primary_schema(table_config: &TableConfig) -> Arc<datafusion_schema::Schema> {
        ...
        for field in table_config.fields.iter() {
    ...
    pub fn extract_timestamp(table_config: &TableConfig, document: &JsonValue) -> Result<i64> {
        let ts_nanoseconds = document
            .dot_get::<JsonValue>(table_config.timestamp.as_str())?
    ...
    pub fn extract_field_values(
        ...
        for field in table_config.fields.iter() {
            let field_value = extract_field_value_from_json_value(&document, &field)?;
    ...
    pub fn extract_label_values_as_object(
        ...
        if table_config.labels.len() == 0 {
            return Ok(document.clone());
        }

        let mut object = serde_json::json!({});
        for label in table_config.labels.iter() {
            let value = match document.dot_get::<JsonValue>(label.as_str())? {
                Some(v) => v,
                None => continue,
            };
    ...
    pub fn extract_tag_values(
        ...
        let mut tags = Vec::with_capacity(table_config.tags.len());
        for tag in table_config.tags.iter() {
            if let Some(value) = document.dot_get::<JsonValue>(tag.as_str())? {
