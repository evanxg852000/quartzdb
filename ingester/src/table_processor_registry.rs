...
impl TableProcessorRegistry {
    pub async fn try_new(capacity: usize, storage: Arc<dyn Storage>, storer_client: StorerClient, metastore_client: MetastoreClient) -> Result<Self> {
        let tables = metastore_client.list_tables().await?;
        let processors = stream::iter(tables)
            .map(|table_meta| async {
                let processor = Self::create_processor(table_meta, storage.clone(), storer_client.clone()).await?;
    ...
    pub async fn get_processor(&self, table_name: &str) -> Result<Arc<TableProcessor>> {
        ...
        let processor = match entries.get(table_name) {
            ...
            None => {
                ...
                let processor = Self::create_processor(table_meta, self.storage.clone(), self.storer_client.clone()).await?;
    ...
    async fn create_processor(table_meta: TableMeta, storage: Arc<dyn Storage>, storer_client: StorerClient) -> Result<Arc<TableProcessor>> {
        let context = Arc::new(IngesterContext::new(Arc::new(table_meta), storage, storer_client));
